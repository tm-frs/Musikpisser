...
/* Information about afterAdd functions:
  ...
  * - it should change the volume to something with `queue.node.setVolume(volume);`
  * - default volume can be used by pasting `queue.node.setVolume(client.config.opt.discordPlayer.initialVolume);` at the end
...
const rasputinprep = async (client, interaction, queue, QueueRepeatMode) => {
	...
	await wait(1); // Wait for 0.001 seconds
	// volume:
	queue.node.setVolume(250);
	interaction.followUp({ content: `Volume changed to **250%** (maximum is **${client.config.opt.maxVol}%**) 🔊` }).catch((e) => {}); // eslint-disable-line no-unused-vars
};
...
const wideputinprep = async (client, interaction, queue, QueueRepeatMode) => {
	...
	await wait(1); // Wait for 0.001 seconds
	// volume:
	queue.node.setVolume(200);
	interaction.followUp({ content: `Volume changed to **200%** (maximum is **${client.config.opt.maxVol}%**) 🔊` }).catch((e) => {}); // eslint-disable-line no-unused-vars
};
