...
module.exports = {
	...
	run: async (client, interaction) => {
		...
		if (!name) return interaction.reply({ content: `Please enter a valid song name. ❌`, ephemeral: true }).catch((e) => { }); // eslint-disable-line no-unused-vars
		...
		if (!res || !res.tracks.length) return interaction.reply({ content: `No search results were found. ❌`, ephemeral: true }).catch((e) => { }); // eslint-disable-line no-unused-vars
		...
		interaction.reply({ embeds: [embed], components: ui }).catch((e) => { }); // eslint-disable-line no-unused-vars
	}
};
